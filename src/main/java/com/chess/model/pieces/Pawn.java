...
public class Pawn extends Piece{
    ...
    public List<Square> getLegalMoves(Piece[][] board, int row, int col) {
        ...
        if (row == startingRow && board[row + moveDirection][col] == null && board[row + moveDirection * 2][col] == null) {
            moves.add(new Square(row + moveDirection*2, col));
        }

        if (isInsideBoard(row+moveDirection, col+1) && board[row + moveDirection][col+1] != null && board[row + moveDirection][col+1].getPieceColor()!= this.getPieceColor()) {
