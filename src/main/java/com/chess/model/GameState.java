...
System.out.println("Board before capture: " + board[from.getRow()][to.getCol()]);
    ...
    private List<Move> getEnPassantMoves(PieceOnSquare pieceOnSquare){
        ...
        if (from.getRow() != (pieceOnSquare.getPiece().getPieceColor() == PieceColor.WHITE ? 3 : 4)) return moves;
        ...
        if (lastMove.getTo().equals(leftCaptured))captured = leftCaptured;
        else if (lastMove.getTo().equals(rightCaptured)) captured = rightCaptured;
        else return moves;
        System.out.println("ENPAssANT CHECL");
        if (!lastMove.getFrom().equals(new Square(captured.getRow() + direction * 2, captured.getCol()))) return moves;
        if (board[lastMove.getTo().getRow()][lastMove.getTo().getCol()].getPieceType() != PieceType.PAWN) return moves;
        moves.add(new Move(from, new Square(captured.getRow() + direction, captured.getCol()), this, MoveType.EN_PASSANT));
        return moves;
    }
    ...
    private List<Move> getPromotionMoves(PieceOnSquare pieceOnSquare){
        ArrayList<Move> moves = new ArrayList<>();
        Square from = pieceOnSquare.getSquare();
        if (from.getRow() != (pieceOnSquare.getPiece().getPieceColor() == PieceColor.WHITE ? 1 : 6)) return moves;
        int direction = pieceOnSquare.getPiece().getPieceColor() == PieceColor.WHITE ? -1 : 1;
